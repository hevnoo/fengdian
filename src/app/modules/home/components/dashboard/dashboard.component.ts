...
export class DashboardComponent extends PageComponent implements IDashboardComponent, DoCheck, OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  @Input()
  widgetLayouts: WidgetLayouts; // 微件布局
  ...
  ngOnInit(): void {
    ...
    this.gridsterOpts = {
      gridType: GridType.ScrollVertical,// 垂直滚动条 水平没有滚动条
      keepFixedHeightInMobile: true,// 在手机端保持固定高度
      disableWarnings: false, // 禁用有关网格项位置错误的控制台日志警告
      disableAutoPositionOnConflict: false,// 在冲突状态下禁用项目的自动定位
      pushItems: false, // 在item调整大小和拖动时是否可以影响其他的item的位置或大小;
      swap: false,// swap互换 允许项目切换位置，如果落在另一个
      maxRows: 100, // 网格中的最大行数
      minCols: this.columns ? this.columns : 24, // 网格中列的最小数量
      maxCols: 3000,// 网格中列的最大数量
      maxItemCols: 1000, // item所占最大列数
      maxItemRows: 1000, // item所占最大行数
      maxItemArea: 1000000, // 最大项目区域: 列 * 行 相乘
      outerMargin: true, // 网格外边距
      margin: isDefined(this.margin) ? this.margin : 10, // 外边距
      minItemCols: 1, //item 最少所占的列数
      minItemRows: 1,//item 最少所占的行数
      defaultItemCols: 8,// 默认8个网格
      defaultItemRows: 6,// 默认6个网格
      resizable: {enabled: this.isEdit}, // item是否可以调整大小
      draggable: {enabled: this.isEdit}, // enabled是否可以拖动item
      itemChangeCallback: item => this.dashboardWidgets.sortWidgets(), // 当改变x, y, rows, cols时，调用每个item
      itemInitCallback: (item, itemComponent) => { // 初始化时为每个项调用的回调函数
