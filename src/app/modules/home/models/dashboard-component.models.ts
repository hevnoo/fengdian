...
export class DashboardWidget implements GridsterItem, IDashboardWidget {
  ...
  get layerIndex(): number {
    let res;
    if (this.widgetLayout) {
      if(typeof this.widgetLayout.layerIndex === 'undefined') {
        res = 1;
        return;
      }
      res = this.widgetLayout.layerIndex;
    } else {
      res = this.widget.layerIndex;
    }
    return Math.floor(res);
  }

  set layerIndex(x: number) {
    if (!this.dashboard.isMobileSize) {
      if (this.widgetLayout) {
        this.widgetLayout.layerIndex = x;
      } else {
        this.widget.layerIndex = x;
      }
    }
  }
