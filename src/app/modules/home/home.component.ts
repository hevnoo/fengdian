...
export class HomeComponent extends PageComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit() {
    ...
    // 获取菜单数据 超管无需获取
    if (this.authState.authUser.authority === Authority.SYS_ADMIN) return;
    this.TabsService.getMenuData().then((menu: any) => {
      if (menu.length > 0) {
        const id = menu[0].dashboardId ?? menu[0]?.children[0]?.dashboardId;
        this.dashboardlink = `/front/dashboard/${id}`
      }
    })
  }
